...
**Q: 回答写到一半就停了？**
A: 两种原因，看输出区下方的提示或接口的 `finish_reason`：

| 现象 | 原因 | 处理 |
|------|------|------|
| `⚠️ 回答达到长度上限` / `finish_reason: length` | 单次生成超过 `MAX_TOKENS`（2048，在 `adapters/nlp.py` 里） | 让它分段写，或调大该常量 |
| `推理失败: 输入过长：N tokens 已超出…` | 输入本身超过 4096 的上下文 | 缩短输入 |

注意 `MAX_TOKENS` 是**上限而非目标**，答完即停 —— 调大它不会让「你好」这种短问答变慢，
只让长代码有机会写完。2048 tokens 约合 4000 汉字，实测一个完整的 PHP 函数（含注释与
示例调用）约 916 tokens。

上下文 4096 由 `docker-compose.yml` 里 `nlp` 服务的 `-c` 决定，4GB 卡上不能调大；
但 `max_tokens` 要得再多也不会报错 —— llama.cpp 生成到剩余空间用尽时自己以 `length`
收尾（实测 prompt 2513 + max_tokens 2048 能正常返回，生成 1583 tokens 后停）。
